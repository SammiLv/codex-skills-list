...
- **不得包含 AI 相关议题**：如果 `吕夏苗钉钉周工作总结（mcp）` 第 1-3 部分中包含 AI 相关议题（如 AI 工具使用、AI 工作流搭建、skill 管理、Codex 沙箱排查等），必须从第 1-3 部分移除；第 4 部分仍只能从 `AI工具一周使用总结` 归纳，不得用钉钉周工作总结补充
- 不得将其他团队成员的周报内容混入此节
